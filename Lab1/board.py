    ...
    def __init__(self):
        ...
        for i in range(self.__height):
            for j in range(self.__width):
                self.__surface[i][j] = Constants.UNKNOWN_POSITION
    ...
    def markDetectedWalls(self, e, x, y):
        ...
        if walls[Constants.UP] > 0:
            while i >= 0 and i >= x - walls[Constants.UP]:
                self.__surface[i][y] = Constants.EMPTY_POSITION
                i = i - 1
        if i >= 0:
            self.__surface[i][y] = Constants.WALL
        ...
        if walls[Constants.RIGHT] > 0:
            while j < self.__width and j <= y + walls[Constants.RIGHT]:
                self.__surface[x][j] = Constants.EMPTY_POSITION
                j = j + 1
        if j < self.__width:
            self.__surface[x][j] = Constants.WALL
